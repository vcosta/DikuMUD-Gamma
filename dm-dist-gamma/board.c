...
void board_write_msg(struct char_data *ch, char *arg) {
	...
	if (!head[msg_num]) {
		error_slog("Malloc for board header failed.\n\r");
...
void board_save_board() {
	...
	int ind, len;
	if (!msg_num) {
		error_slog("No messages to save.\n\r");
		return;
	}
	the_file = fopen(SAVE_FILE, "wb");
	if (!the_file) {
		error_slog("Unable to open/create savefile..\n\r");
		return;
	}
...
void board_load_board() {
	...
	if (!the_file) {
		error_slog("Can't open message file. Board will be empty.\n\r",0);
		return;
	}
	fread(&msg_num, sizeof(int), 1, the_file);
	
	if (msg_num < 1 || msg_num > MAX_MSGS || feof(the_file)) {
		error_slog("Board-message file corrupt or nonexistent.\n\r");
		fclose(the_file);
		return;
	}
	for (ind = 0; ind < msg_num; ind++) {
		...
		if (!msgs[ind]) {
			error_slog("Malloc for board msg failed..\n\r");
			board_reset_board();
			fclose(the_file);
			return;
		}
...
void error_slog(char *str) {	/* The original error-handling was MUCH */
