...
void load_char_objs(struct char_data *ch)
{
	...
	if (found) {
		...
		if (fwrite(&st, sizeof(struct obj_file_u), 1, fl) < 1) {
			logstr("Error updating name to be set as unused.");
			exit(1);
		}

	} else {
		logstr("Char has no data in file!");
	}
...
void put_obj_in_store(struct obj_data *obj, struct obj_file_u *st)
{
	...
	if (!found) {
		logstr("No empty space to store object. (put_obj_in_store, reception.c)");
		exit(1);
	}
}
